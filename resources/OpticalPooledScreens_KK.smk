...
true_cell_barcodes=pd.read_csv( config['BARCODE_TABLE'] )['barcode'].to_list()
...
rule filter_reads:
    input:
        processed_input('reads.csv')
    output:
        processed_output('filtered_reads.csv')
    run:
        Snake.filter_reads(output=output, df_reads=input[0], true_cell_barcodes=true_cell_barcodes)

rule call_cells_filtered_reads:
    input:
        processed_input('filtered_reads.csv')
    output:
        processed_output('filtered_cells.csv')
    run:
        Snake.call_cells(output=output, df_reads=input[0])

rule plot_reads_on_cells_outline:
    input:
        processed_input('filtered_reads.csv'),
        processed_input('nuclei.tif'),
        processed_input('cells.tif'),
    output:
        processed_output('show_reads.pdf')
    run:
        Snake.plot_reads_on_cells_outline(output=output, df_reads=input[0], nuclei=input[1], 
            cells=input[2], true_cell_barcodes=true_cell_barcodes)
...
rule merge_cell_tables:
    input:
        cells=expand(processed_input('filtered_cells.csv'), zip, well=WELLS, tile=TILES),
